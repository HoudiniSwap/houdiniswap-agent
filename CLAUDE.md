...
`plugins/houdiniswap/skills/houdiniswap/SKILL.md` is a system-prompt-style instruction doc shipped to end agents — the skill bundled in the `houdiniswap` Claude Code plugin (distributed via the `.claude-plugin/marketplace.json` marketplace in this repo; manual copy to `~/.claude/skills/` also works). It is **not** wired into any MCP resource. It documents idealized behavior and can lag the real tool signatures, so **when the two disagree, check both against the backend controllers in `houdiniswap-backend/src/tsoa/controllers/` — that is the only source of truth.**

This paragraph previously claimed the tools take `address` and `quoteId` while SKILL.md says `addressFrom` and `id`. That was backwards, and following it would have reverted the fix for a bug where every DEX call returned 422. The tools take `addressFrom` (`dex.ts`) and `id` for `dexConfirmTx`, matching both SKILL.md and the API. `packages/ai-tools/__tests__/parity.test.ts` pins these names against the API contract; if you change a tool parameter, update SKILL.md and that test together.
